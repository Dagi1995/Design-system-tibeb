...
export interface GroupedRow<T extends RowData> {
  group: string;
  collapsed: boolean;
  rows: T[];
}
...
function ListView<T extends RowData>({
  className,
  columns,
  rows,
  showFooter = false,
}: TableProps<T>) {
  const [tableRows, setTableRows] = React.useState<T[] | GroupedRow<T>[]>(rows);
  const isEmpty = tableRows.length === 0;
  const isGrouped =
    Array.isArray(rows) && rows.length > 0 && "group" in (rows[0] as object);
  ...
  const handleResizeStart = (
    ...
    setResizeData({
      ...
      startX: e.clientX,
      startWidth:
        typeof columnWidths[columnKey] === "number"
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      const allRowIds = isGrouped
        ? (tableRows as GroupedRow<T>[]).flatMap((group) =>
            group.rows.map((row) => row.id)
          )
        : (tableRows as T[]).map((row) => row.id);
  ...
  const handleAddRow = () => {
    if (isGrouped) return; // skip for grouped mode

    const newRow = {
      id: Math.max(0, ...(tableRows as T[]).map((r) => r.id)) + 1,
      no: String((tableRows as T[]).length + 1),
      name: "New User",
    ...
    setTableRows((prev) => [...(prev as T[]), newRow]);
  };

  const handleDeleteSelected = () => {
    setTableRows((prev) =>
      isGrouped
        ? (prev as GroupedRow<T>[]).map((group) => ({
            ...group,
            rows: group.rows.filter((row) => !selectedRows.has(row.id)),
          }))
        : (prev as T[]).filter((row) => !selectedRows.has(row.id))
    );
                  ...
                  <TableHead className="h-12 w-12 pl-4">
                    <Checkbox
                      checked={
                        selectedRows.size ===
                        (isGrouped
                          ? (tableRows as GroupedRow<T>[]).reduce(
                              (sum, group) => sum + group.rows.length,
                              0
                            )
                          : (tableRows as T[]).length)
                      }
